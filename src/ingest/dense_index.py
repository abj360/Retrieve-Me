...
class DenseIndex:
    ...
    def drop_collection(self) -> None:
        """Drops the collection when it exists; a no-op when it does not."""
    ...
    def upsert(
        ...
        if not points:
            ...
            return 0
        logger.info("upserting %d points (batch=%d)", len(points), batch_size)
        with self.pool.acquire() as client:
    ...
    def health_check(self) -> bool:
        """Reports whether Qdrant answers a collections listing within the timeout (no retry).
